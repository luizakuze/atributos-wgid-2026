...
class SAMLAgregacaoIdPUser(HttpUser):
    """
    Cenario B - Agregacao pelo IdP (Figura 1b do artigo).
    O IdP autentica o usuario e consulta a Autoridade de Atributos (AA)
    antes de gerar a assercao SAML, agregando os atributos institucionais
    aos recebidos da AA. Fluxo com 7 mensagens (Tabela 1).
    """
    ...
    @task
    def fluxo_agregacao_idp_1_a_7(self):
        ...
        try:
            ...
            fire_custom(
                self.environment,
                "4 - IdP autentica o usuario e consulta a AA",
                step_ms,
                len(r4.content or b""),
            ...
            try:
                ...
                aa_len = len(r5.content or b"")

                if not r5.ok:
                    aa_exception = Exception(f"AA HTTP {r5.status_code}")
            ...
            fire_custom(
                self.environment,
                "5 - AA retorna ao IdP os atributos do usuario",
                step_ms,
                aa_len,
                aa_exception
            )
            ...
            already_authenticated = (
                "Atributos recebidos" in r4.text
                or "SP SAML Demo" in r4.text
                or "IdP selecionado" in r4.text
            )

            if not saml_form and already_authenticated:
                step_ms = (time.time() - step_start) * 1000

                fire_custom(
                    self.environment,
                    "6 - IdP agrega atributos e gera a assercao SAML",
                    step_ms,
                    len(r4.content or b""),
                    None
                )
                ...
                if not saml_form:
                    ...
                    debug_page("Falha no passo 6 - resposta apos login", r4)
                    return
                ...
                fire_custom(
                    ...
                    step_ms,
                    len(r6.content or b""),
                    None if r6.ok else Exception(f"HTTP {r6.status_code}")
                )

            step_start = time.time()

            if (
                "Atributos recebidos" in r6.text
                or "SP SAML Demo" in r6.text
                or "IdP selecionado" in r6.text
            ):
                r7 = r6
            else:
                r7 = session.get(
            ...
            final_ok = (
                "Atributos recebidos" in r7.text
                or "SP SAML Demo" in r7.text
                or "IdP selecionado" in r7.text
            )

            fire_custom(
                self.environment,
                "7 - SP recebe a assercao e decide o acesso",
                step_ms,
                len(r7.content or b""),
            ...
            if not final_ok:
                debug_page("Falha no passo 7", r7)
                return
            ...
            print("LOGIN OK - fluxo 1 a 7 completo")
