...
AA_BASE_URL = "http://aa-api:8000"
AA_TIMEOUT_SECONDS = 3
...
SCENARIO_LABEL = "Cenário C · Agregação pelo SP"
SCENARIO_SUBTITLE = "O SP consulta a Autoridade de Atributos depois de receber a asserção do IdP"
FLOW_STEPS = ["SP", "DS", "IdP", "SP", "AA", "SP"]
...
def index():
        ...
        <body style="min-height: 100vh; display: flex; align-items: center; justify-content: center; padding: 24px;">
            <div class="card" style="max-width: 640px; width: 100%; padding: 40px;">
                ...
                <p class="note" style="margin-bottom: 24px;">
                    Na agregação pelo SP, o IdP envia a asserção inicial e o
                    próprio SP consulta a AA para obter atributos
                    complementares antes da autorização.
                </p>
    ...
    aa_sourced_attributes = set(session.get("aaSourcedAttributes", []))
    ...
    <body>
        <header style="background: linear-gradient(135deg, #15803d, #16a34a); color: white; padding: 32px 24px;">
        ...
        <main style="max-width: 900px; margin: 0 auto; padding: 24px;">
            ...
            <div class="card">
                <p class="eyebrow">Atributos após agregação pelo SP (IdP + AA)</p>
                <table>
                    <thead>
                        <tr>
                            <th>Atributo</th>
...
def fetch_aa_attributes(uid):
    """
    Cenário C: agregação pelo SP. Consulta a Autoridade de Atributos
    (AA) diretamente pelo SP para obter atributos complementares
    (ex.: eduPersonEntitlement), que o IdP não incluiu na asserção.
    """
    if not uid:
        return {}
    ...
    try:
        response = requests.get(
            f"{AA_BASE_URL}/attributes/{uid}",
            timeout=AA_TIMEOUT_SECONDS,
        )
    except requests.RequestException as exception:
        print(f"\n=== FALHA AO CONSULTAR A AA ({uid}): {exception} ===\n")
        return {}

    if response.status_code == 404:
        print(f"\n=== AA NÃO POSSUI ATRIBUTOS PARA O USUÁRIO: {uid} ===\n")
        return {}

    if not response.ok:
        print(f"\n=== AA RETORNOU HTTP {response.status_code} PARA {uid} ===\n")
        return {}
...
def acs():
    ...
    # "FriendlyName" (ex.: "uid") - fetch_aa_attributes() abaixo depende
    # de achar a chave "uid" pra funcionar.
    saml_attributes = auth.get_friendlyname_attributes()
    name_id = auth.get_nameid()

    print("\n=== ATTRIBUTES RECEIVED FROM IDP/SHIBBOLETH (BÁSICOS) ===")
    print(json.dumps(saml_attributes, indent=2, ensure_ascii=False))
    print("=== END ATTRIBUTES RECEIVED FROM IDP/SHIBBOLETH ===\n")

    uid_values = saml_attributes.get("uid") or []
    uid = uid_values[0] if uid_values else None

    aa_attributes = fetch_aa_attributes(uid)

    aggregated_attributes = dict(saml_attributes)
    for name, values in aa_attributes.items():
        aggregated_attributes[name] = values

    # Guarda quais chaves vieram de verdade da AA nesta consulta, pra
    # tag "via AA" não aparecer em atributos que o IdP já liberou
    # nativamente (ex.: eduPersonEntitlement de um IdP real que não
    # depende da nossa AA).
    session["aaSourcedAttributes"] = list(aa_attributes.keys())

    print("\n=== ATTRIBUTES AFTER SP-SIDE AGGREGATION (AA) ===")
    print(json.dumps(aggregated_attributes, indent=2, ensure_ascii=False))
    print("=== END ATTRIBUTES AFTER SP-SIDE AGGREGATION ===\n")

    session["samlUserdata"] = aggregated_attributes
