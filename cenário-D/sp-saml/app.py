...
# Cenário D: agregação pelo proxy. Independentemente do IdP escolhido no
# Discovery Service, o SP autentica sempre contra o proxy de identidade
# (SATOSA), que concentra a autenticação federada e a agregação de
# atributos antes de encaminhar a asserção final a este SP.
PROXY_IDP_ENTITY_ID = "https://proxy-wgid.gidlab.rnp.br/Saml2IDP/proxy.xml"

# A tag "via AA" só é aplicável quando o IdP de origem escolhido no DS é
# o nosso: a AA mock deste ambiente só tem entradas pro nosso usuário de
# teste. IdPs reais da federação podem liberar eduPersonEntitlement
# nativamente, o que não tem relação com a AA consultada pelo proxy.
...
SCENARIO_LABEL = "Cenário D · Agregação pelo proxy"
SCENARIO_SUBTITLE = "O proxy de identidade fica entre o IdP e o SP, e consulta a Autoridade de Atributos para agregar os atributos antes de entregar a asserção final ao SP"
FLOW_STEPS = ["SP", "DS", "Proxy", "IdP", "Proxy", "AA", "Proxy", "SP"]
...
def index():
        ...
        <body style="min-height: 100vh; display: flex; align-items: center; justify-content: center; padding: 24px;">
            <div class="card" style="max-width: 640px; width: 100%; padding: 40px;">
                ...
                <p class="note" style="margin-bottom: 24px;">
                    Na agregação pelo proxy, uma camada intermediária
                    consulta a AA, agrega os atributos e entrega ao SP uma
    ...
    real_selected_idp = session.get("realSelectedIdP", "")
    aa_sourced_attributes = AA_SOURCED_ATTRIBUTES if real_selected_idp == OWN_IDP_ENTITY_ID else frozenset()
    ...
    <body>
        <header style="background: linear-gradient(135deg, #854d0e, #a16207); color: white; padding: 32px 24px;">
...
def ds_callback():
    ...
    # Cenário D: a AuthnRequest do SP sempre é endereçada ao proxy (é ele
    # quem de fato conversa com o IdP de origem e agrega os atributos da
    # AA) - mas o IdP escolhido aqui no DS não é descartado: é repassado
    # ao proxy logo abaixo, via idp_hint, para que ele autentique contra
    # esse mesmo IdP em vez de cair na própria descoberta.
    proxy_idp = PROXY_IDP_ENTITY_ID

    try:
        idp = find_idp_in_federation_metadata(proxy_idp)
    except Exception as exception:
        return f"Proxy não encontrado no metadata local: {html.escape(str(exception))}", 400

    session["selectedIdP"] = proxy_idp
    session["realSelectedIdP"] = selected_idp

    print("\n=== IDP (PROXY) LOADED FROM FEDERATION METADATA ===")
    print(f"IdP escolhido no DS: {selected_idp}")
    print(f"EntityID usado (proxy): {idp['entity_id']}")
    print(f"SSO URL: {idp['sso_url']}")
    print(f"Signing cert length: {len(idp['signing_cert'])}")
    print("=== END IDP (PROXY) LOADED FROM FEDERATION METADATA ===\n")

    req = prepare_flask_request(request)
    auth = init_saml_auth(req, proxy_idp)

    login_url = auth.login(return_to=SP_HOME_URL)

    # Repassa o IdP escolhido na WAYF como dica pro proxy (idp_hint), lido
    # pelo microservice IdpHinting do SATOSA: o proxy autentica direto
    # contra esse IdP em vez de mostrar sua propria tela de descoberta.
    # Sem isso, o proxy so mostra a descoberta quando ha mais de um IdP na
    # metadata dele - e sempre autentica contra o unico IdP quando so ha um.
    separator = "&" if "?" in login_url else "?"
    login_url = f"{login_url}{separator}{urlencode({'idp_hint': selected_idp})}"
...
def acs():
    ...
    # "FriendlyName" quando disponível. No cenário D isso ainda depende de
    # o SATOSA anexar FriendlyName na resposta (pendência separada,
    # documentada no README) - o proxy não usa esse dict pra nada, só o SP
    # exibe.
